class Voodoo < Formula
  ...
  url "https://files.pythonhosted.org/packages/source/v/voodoo-framework/voodoo_framework-1.17.0.tar.gz"
  ...
  def install
    ...
    # Pin the exact version — an unpinned `uv tool install` would pull whatever
    # is latest on PyPI at `brew install` time and ignore the url/sha256 above,
    # shipping stale framework code.
